...
parser.add_argument('--output_path', type=str, default='/raid/compass/backup/output_long/', help="output path")
...
while True:
	for it, (batch_a, images_b) in enumerate(zip(train_loader_a, train_loader_b)):
		...
		with Timer("Elapsed time in update: %f"):
			...
			# x = torch.randn(1, 1, 512, 512).requires_grad_(True)  # Design an Input
			# y = trainer(x)
			# # y=trainer.gen_update(batch_a, images_b, config)
			# trainerVis = make_dot(y, params=dict(list(trainer.named_parameters()) + [('x', x)]))
			# trainerVis.format = "png"
			# # The saved file
			# trainerVis.directory = "/home/longchen/surgical-video-sim2real-master/translation_model/"
