...
class StylelessGen_LC_encoder_decoder(nn.Module):
    # AdaIN auto-encoder architecture
    def __init__(self, input_dim, params):
        ...
        activ = params['activ']
        pad_type = params['pad_type']
        self.encoder_decoder = Encoder_Decoder(n_downsample, n_res, input_dim, dim, norm='in', activ=activ, pad_type=pad_type)

    def forward(self, images):
        # reconstruct an image
        images_recon = self.encode_decoder(images)
        return images_recon
...
class StylelessGen(nn.Module):
    ...
    def forward(self, images, LBPimages):
        # reconstruct an image
        content = self.encode(images)
        images_recon = self.decode(content, LBPimages)
        return images_recon
    ...
    def decode(self, content, LBPimages):
        # decode content codes to an image
        images = self.dec(content, LBPimages)
        return images

class StylelessGen_ORI(nn.Module):
    # AdaIN auto-encoder architecture
    def __init__(self, input_dim, params):
        super(StylelessGen, self).__init__()
        dim = params['dim']
...
class MLP(nn.Module):
    ...
    def forward(self, x):
        aaa=self.model(x.view(x.size(0), -1))
